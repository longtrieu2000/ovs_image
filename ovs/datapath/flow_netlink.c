...
/* Viet: Tinh tong kich thuoc netlink attributes cho OVS_TUNNEL_KEY (su dung khi encode). */
...
/* Viet: Tinh kich thuoc buffer can thiet de luu toan bo OVS_KEY_* netlink attributes. */
...
/* Viet: Parse danh sach Netlink nested OVS_KEY_ATTR_* thanh mang con tro va bitmap attrs (khong phai mask). */
...
/* Viet: Chuyen ip_tunnel_info sang netlink attributes (OVS_KEY_ATTR_TUNNEL) vao skb. */
...
/* Viet: Doc NSH header tu nlattr thanh struct nshhdr (bo gom base va MD1/MD2), tra loi loi neu sai kich thuoc. */
...
/* Viet: Parse NSH key/mask tu nlattr vao ovs_key_nsh va ovs_key_nsh mask. */
 ...
 */
/* Viet: Parse key/mask netlink thanh sw_flow_match; neu khong co mask thi tao exact match (mask full 0xff). */
 ...
 * or false otherwise.
 */
/* Viet: Doc UFID tu nlattr vao sw_flow_id; tra ve true neu hop le. */
...
/* Viet: Lay co UFID (OVS_UFID_F_*) neu attr ton tai, nguoc lai 0. */
...
/* Viet: Dong goi sw_flow_key/output vao nla nest OVS_FLOW_ATTR_KEY/MASK tren skb (ho tro mask hoac key thong thuong). */
...
/* Viet: Ghi identifier cua flow (UFID neu co, neu khong la unmasked key) vao netlink msg. */
...
/* Called with ovs_mutex or RCU read lock. */
/* Viet: Ghi key cua flow vao OVS_FLOW_ATTR_KEY (khong mask). */
...
/* Called with ovs_mutex or RCU read lock. */
/* Viet: Ghi mask cua flow vao OVS_FLOW_ATTR_MASK. */
...
/* Viet: Giai phong struct sw_flow_actions (va cac tai nguyen phu tro trong OVS_ACTION). */
...
/* Viet: Giai phong actions theo RCU sau grace period. */
...
/* Viet: Them mot OVS_ACTION_ATTR_* vao danh sach actions (cap phat mo rong neu can). */
...
/* Viet: Khoi tao sw_flow_match, reset key/mask neu duoc chi dinh. */
...
/* Viet: Sao chep/validate danh sach OVS_ACTION tu netlink vao sw_flow_actions (cap phat buffer moi). */
...
/* Viet: Chuyen danh sach OVS_ACTION_* tu netlink (kernel luu) sang buffer skb de tra ve user. */
